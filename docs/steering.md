...
### R-1: レスポンシブ対応（desktop/mobile 2フレーム構成）

**目的**: デスクトップとモバイルの2フレーム構成に対応し、設計書 4.9 節のディレクトリ構造で出力する。

**前提**: E-4 完了

**作業内容**:
- [ ] ページ内に複数の最上位フレームがある場合の検出ロジック
- [ ] フレーム幅によるデスクトップ/モバイル判定（1440px / 375px）
- [ ] 出力構造の変更:
  ```
  telldes-export/
  ├── prompt.md
  ...
  └── mobile/
      ...
      └── assets/
  ```
- [ ] tokens.json はルート（共通）に配置
- [ ] 単一フレーム時は従来構造を維持（後方互換）
- [ ] セルフチェック（チェック結果: `docs/checks/R-1.md`）
- [ ] ユーザーレビュー依頼・OK取得

**完了条件**:
- 2フレーム構成時に desktop/ と mobile/ が正しく分離出力されること
- tokens.json がルートに1つだけ出力されること
- 単一フレーム時に従来の出力構造が維持されること
- フレーム幅が 1440px / 375px 以外の場合にユーザーに確認する動作があること
