...
そこで telldes は、ガイドと、それに沿った変数とスタイルの一式と、ガイドからのずれを見つけるチェックを用意し、ファイルにある情報を CC がそのまま使える形で書き出す。Figma のデータに無い情報（動き、どの幅で切り替えるか、ページの題名など）は、プラグインで補足として受けて一緒に渡す。それでも無いものは、推測せずに「無い」と書いて渡す。
...
flowchart LR
    ...
    D -->|"Setup・チェック・書き出し"| P["telldes プラグイン"]
    P <-->|"読む・書く"| F
    P -->|"書き出す"| Z[("zip")]
    Z -->|"読む"| C["CC"]
...
- デザイナー: ガイドに従って Figma でデザインする。Figma のデータに無いことを補足として書く。チェックの error を直して書き出す
- Figma のファイル: デザインそのもの。レイヤーの木、変数、スタイル、補足（note）と Export 設定を持つ
- telldes プラグイン: ファイルを読み、チェックし、zip を書き出す。変数とスタイルの一式を作る（Setup）。ライトとダークを切り替える
...
flowchart LR
    ...
    M -->|"出力の中身・チェック結果<br/>付け替えや書き出しの計画"| R
    M -->|"画面に出すもの"| UI["③ 画面"]
    UI -->|"操作"| R
    UI --> Z[("zip を組み立てて渡す")]
```
...
- ② 作る層: 読み取りデータだけから、zip に入るファイルの中身、チェック結果、撮る画像の一覧、テーマの付け替えの計画、Setup の計画を作る。Figma に触らない
...
3. ②がチェックする。error があればここで止める
4. ②が zip のファイルの中身と、撮る画像の一覧を作る
5. ①が画像を撮る
6. ダーク対応なら、ダークに付け替えてもう一度撮り、ライトに戻す
7. ③が zip にまとめる

チェックと出力は、2 で読んだ同じデータから作る。別々に読むと、チェックしたものと渡したものがずれうるから。途中で失敗しても、必ずライトに戻す。
...
変数をライト・ダーク・共通の3つのコレクションに分け、ページ全体の変数のつながりをライトとダークの間で付け替える。無償版では1つのコレクションに複数のモードを作れないが、コレクションの数には制限が無い。

ダーク用のフレームを複製しない。複製すると、同じページが別ページとして CC に渡り、ライトの変更がダークに伝わらない。代わりに、ファイルが「今どちらのテーマか」という状態を持つ。書き出しの途中で失敗しても必ずライトに戻し、それでもダークで残った場合は、次に開いたときに戻すよう促す。
...
テストは、zip に入るものの形・チェックの規則・付け替えの計算が約束どおりかだけを確かめる。
...
- ファイルがテーマの状態を持ち、中断でダークのまま残りうる
- キャンバスでライトとダークを並べて比べられない。比べるのは書き出した画像で行う
