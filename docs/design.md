...
| オブジェクト | 正体 | 多重度 |
|---|---|---|
| **画面** | 書き出し対象のトップレベルフレーム。zip の1フォルダ＝CC が作る1ページ（4.7.4） | コレクション |
| **レイヤー** | 画面の中身 | 画面 1 — * レイヤー |

**違反・付け忘れの知らせ・note は、レイヤーのプロパティであってオブジェクトではない。** したがってビューに昇格させない。旧構成の4タブ（Setup / Review / Notes / Export）はこの昇格をしていたため、同じレイヤーのプロパティが別々の箱に入り、次の4症状を生んでいた — note の一覧が Notes と Export の2箇所に要る／付け忘れの知らせは Export に出るが直す作業は Review 側／Export を押すとチェック結果を見せるために Review へ飛ばされる／テーマ切り替えの置き場所がどのタブにも無い。**プロパティを持ち主に戻せば、4症状は原因ごと消える。**

設定（4.7.4.2）・テーマ・zip はオブジェクトではない。設定は Export アクションのパラメータ、テーマはファイルの状態（モード）、zip はアクションの結果である。

**ビューとナビゲーション**

- **画面のコレクションビュー**が UI の主役。各行が1つの画面で、違反・知らせ・note の件数を持つ。この一覧は**そのまま「これから渡すものの明細」**であり、Export 専用のビューを別に持たない
- 行を展開すると、その画面で telldes が言うことのあるレイヤー（違反・知らせ・note のいずれかを持つもの）が並ぶ。**Figma のレイヤーツリーを作り直さない** — 純正のレイヤーパネルがある以上、telldes が出すべきは telldes が何か言うことのあるものだけ
- コレクションの末尾に **「渡されないもの」** の行を置く。4.7.2 の除外物告知と同じ集合で、これまで zip 内 `README.md` にしか出ていなかったものが Figma 上でも見える（原則B「欠けていると分かる」）
- **レイヤーのシングルビュー**は選択に追従し、そのレイヤーの違反・知らせ・note 入力欄を出す。Figma ではキャンバス上の選択がそのままオブジェクト選択になるため、直接操作が素で成立する。逆に一覧の行をクリックすれば選択が変わる（4.7.3）
...
| アクション | 対象 | 置き場所 |
|---|---|---|
| Export（4.7.4） | 画面コレクション | コレクションビューの見出し |
...
常設するのは **テーマの現在状態**と **error 件数**の2つ。`figma.notify()` との線引きは**消えてよいかどうか**で引く。

| | 例 | 置き場所 |
|---|---|---|
| 状態（持続する事実） | テーマが Light か Dark か / error 件数 / 付け忘れの件数 | UI に領域を持つ |
| 出来事（一度きり） | Export 完了、note 保存、一覧の走査失敗、行の対象が消えた | `figma.notify()` |

テーマ状態の常設が要るのは、telldes が付け替え（4.3.9）で**ファイルの状態を書き換える**ため。利用者に影響のある変更を知らせずに行わない。「起動時に dark のまま残っている」の復旧の促し（4.7.4.4）もここに出る。表示と切り替えを1つの `Light | Dark` にまとめるのは、分けると「今 Dark と書いてあるが切り替えはどこか」になるため。

**寸法**

360 × 640（`figma.showUI` の既定は 300×200、旧構成は 360×480）。コレクションビューとシングルビューを縦に積むため高さが要る。Figma 公式がプラグインに勧める「最小幅 300px」は Dev Mode のインスペクトパネルに収める場合の助言で、フローティングウィンドウである telldes には当てはまらない。違反行（レイヤーパス＋理由）が折り返さずに読める幅を優先する。
...
**実行失敗の扱い:** チェックの実行自体が例外で中断した場合は、その旨をUIに通知し「実行中…」表示を解除する（書き出しの失敗通知と同じ方針）。通知しないと画面が「実行中…」のまま固まり、デザイナーには原因も次の一手も分からない。あわせて**前回の実行結果の表示を消す**。画面が示すのは常に直近1回の実行の結果であり、失敗通知の下に前回の一覧が残っていると、それが今回の検出結果だと読めてしまう。
...
Export はボタンを押した時点でチェックが自動で走り（4.7.2 の原則3）、エラーが1件でもあれば中止して結果を画面に出す（4.7.1）。エラーだけを理由に止めてよいのは、Review の結果が出力の正確さを損なうものだけに絞ってあるから（4.7.2）。
...
**Export ボタンを押すと確認ビューに入る。** 並びは上から **設定 → 共通ルール → 付け忘れの知らせ → 実行**。4.7.4.1 の依頼書の並び（全体 → 個別 → 手渡し）をそのまま画面にする。付け忘れの知らせ（4.7.2）を実行の直前に置くのは、「直してから渡すか、このまま渡すか」をその場で決められるようにするため。**知らせは Export をブロックしない** — error ではないので、押せば書き出される。

**渡すものの明細をこのビューに持たないのは、コレクションビュー（4.7.1）がすでにそれだから。** 確認ビューが受け持つのは、明細に表れない「全体条件」＝設定と共通ルールだけ。ここで明細を繰り返すと、同じ事実が2箇所に出る（原則B「1箇所にだけある」）。
