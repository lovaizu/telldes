...
**ダーク対応ファイル（Light / Dark / Base の3コレクション、4.3.4）の場合**、コレクション名でJSONをグループ化しない。Light・Darkの両コレクションに同名の変数が存在する場合は1つのトークンとして扱い、`$value`を`{ "light": ..., "dark": ... }`の形にする。Baseコレクションの変数（テーマで値が変わらないもの）は単一値のままとする。
...
コレクション名でグループ化する（`{"Light": {"bg": ...}, "Dark": {"bg": ...}}`）と、CCが「Light.bgとDark.bgは同じトークンの2つの値」と認識できず、無関係な2つの変数として誤って出力しかねない。トークン名を最上位キーにし、`$value`がlight/darkのオブジェクトかどうかだけを見て`:root`と`[data-theme="dark"]`の両方に出すか単一に出すかを機械的に判断できる形にする。Darkコレクションが存在しないファイル（ダーク対応していないファイル）では常に単一値になる。
