...
- 同一カテゴリ内で内容が完全に一致する項目は1行にまとめる。レイヤーパスの**先頭セグメントは、そのレイヤーを含む画面フレームのzipフォルダ名そのもの**とする（4.5.2 のフォルダ名規約 ― パス区切り文字の無害化と重複名への `-N` 付与 ― を適用した結果の文字列。フォルダ名とREADMEのパスは同一の関数で生成し、両者が食い違うことをあり得なくする）。先頭セグメント以外は4.5.2の `path` と同じく同名の兄弟を `-N` で区別する。画面フレーム名を含む点がspec.jsonの`path`と異なるのは、READMEがデザイナーにzip内のフォルダとFigmaのレイヤーツリーの両方を辿らせるための表記であるため
