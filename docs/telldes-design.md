...
**note一覧:** 入力欄はノードを1つ選択して初めて1件のnoteを見せるため、それだけでは「このページのどこに何を書いたか」を確かめる手段がレイヤーを1つずつ選び直すことしかない。そこで入力欄の下に、現在のページでnoteが設定済みの全レイヤーを一覧表示する。

- **一覧の各項目**: ルート（ページ直下）からのレイヤーパスとnote本文の両方を示す。レイヤーパスだけではnoteの中身が分からず、note本文だけではどのレイヤーの話か分からない。パスは ` > ` 区切りで、Figma上の生のレイヤー名をそのまま並べる（4.5.2のフォルダ名正規化も `-N` 付与も行わない。一覧はzipと突き合わせるための表記ではなく、Figmaのレイヤーツリーを辿るための表記であるため）
- **クリック**: 該当ノードを選択し、ビューポートをそこへスクロールする（Reviewの結果一覧と同じ挙動）
- **走査範囲**: ページ全体。noteはどのレイヤーにも付けられるため、書き出し対象外のノード（ページ直下に裸で置かれたComponent定義の内部など）に付いたnoteも一覧に出す。出さなければ、書いたはずのnoteが見当たらないという状態になる。並び順はレイヤーツリーの走査順
- **更新タイミング**: プラグイン起動時と、note保存時（空文字保存＝削除を含む）。選択の変更では更新しない（一覧は選択に依存しない）
- **入力欄との関係**: 選択中ノードの入力欄は一覧の上に残し、ノード未選択でも一覧は表示する
- **0件時**: noteが1件も設定されていない旨のメッセージを出す
