...
- インスタンス内部のノードは走査しない（`isInsideInstance`）。原因は主コンポーネント側にあり、同じ部品をN個配置しただけで同一内容のN行が出るのを防ぐ。Reviewの各チェックがインスタンス内部を除外するのと同じ理由
...
- 同一カテゴリ内で内容が完全に一致する項目は1行にまとめる。レイヤーパスは同名の兄弟を `-N` で区別する（4.5.2の一意性規約と同じ規則。ただしパスには画面フレーム名を含める点がspec.jsonの`path`と異なる ― READMEはデザイナーがFigmaのレイヤーツリー上で当該レイヤーを見つけるための表記であるため）
