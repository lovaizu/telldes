...
- [ ] 設計書の役割を決め直し、それに合わせて書き直す（役割の案をユーザーに確認中。合意済みの「設計書が正」を変えるため）

**完了条件**:
- ②の層が受け取るデータの形が設計書にあり、出力・チェック・付け替えのどれもそれだけで作れること
- テストの目的・仕組みが設計書にあること
- CSS 変数名の規則が設計書・`prompt.md`・Setup で一致していること
- 既存テストが残っていないこと
...
**目的**: Figma から出力・チェック・付け替えに要る情報をすべて読み、B-0 で決めた形のデータにまとめる。
...
**目的**: 読んだデータから spec.json・tokens.json・README・settings.json・prompt.md・steering.md を作る（設計書 4.5・4.8）。

**前提**: B-2 完了

**作業内容**:
- [ ] ②の層として実装する（Figma に触らない）
- [ ] 見本を入力に、出力を正解と丸ごと比べるテストを書く。正解は設計書 4.5 と突き合わせて作る
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-3.md`）

**完了条件**:
- 見本から作った出力が正解と一致し、正解が設計書 4.5・4.8 のフォーマットに合っていること

---

### B-4: チェック

**目的**: 違反・テーマ整合・付け忘れの知らせ・除外物を、設計書 4.7.2 の原則1〜3 どおりに出す（旧 I-4・I-7・U-3 を含む）。
...
**完了条件**:
- 実機で書き出した画像が設計書 4.5 の命名・形式どおりで、`.ico` がブラウザで favicon として表示できること

---

### B-6: テーマ切り替え

**目的**: Light ⇄ Dark の付け替え（設計書 4.3.9）。
...
**目的**: 設計書 4.7.1 の画面構成（トークン・画面・レイヤーの3ビュー、常設ヘッダ、Export 設定と確認ビュー、Setup）を作る（旧 I-2・I-5 の UI 部分・U-2）。
...
- [ ] B-0 で設計書に足したもの: グラデーション・複数 fill の Color Style と Effect Style のトークン、tokens.json の出力条件、非表示のレイヤー（除外物）、線（strokes）、塗り・線の画像のファイル名（`{パス}--fill-{番号}` / `{パス}--stroke`）、新しい告知の種類（再現できない描画・決められない値）
- [ ] 設計書 4.3 / 4.5 / 4.7 と突き合わせる
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-8.md`）

**完了条件**:
- README だけを読んで Setup から Export まで操作でき、設計書と食い違う記述が無いこと
...
* **Status**: paused
* **Date**: 2026-09-25
* **Last completed**: B-0 の設計書作業（4.7.6 3層・4.7.7 テスト・4.5.1 CSS 変数名・描き方の規則、レビュー済み。最終 `d879d2c`）。ユーザー確認だけが残る
* **Next**: B-0 の「ユーザーが設計書の中身を確認する」— ユーザーの `/rn:ty` / `/rn:gm` を待つ。承認なら B-0 をチェックオフ（`complete task #B-0`）して B-1 へ
* **Notes**: 読んでもらう順は 4.7.6 → 4.7.7 → 4.5.1 → 4.5.2.1。レビューと直しは上限3回を1回超えた（退行1件の修正。checks/B-0.md に記録）
