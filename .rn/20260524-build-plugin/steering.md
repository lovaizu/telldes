...
- [ ] **未決1〜5 をユーザーと決着させる（1件ずつ出す）**。未決1（Notes タブの一覧を残すか Export に一本化するか）の結論に応じて U-2 の残タスクを定義し直す。未決3〜5（Setup のトークン数と Dark の扱い／Font family の除外物告知／Typography の段階数）は D-3 で発覚した設計書内の矛盾で、**放置すると実装が食い違うまま進む**
