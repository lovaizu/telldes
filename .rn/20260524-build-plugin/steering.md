...
13. ~~**Review の走査範囲を書き出し対象フレーム配下に絞るか**~~（レビュー #11）→ **決着（2026-09-21）: 書き出されるものだけを見る。** 根拠 — 原則1 の判定軸は「出力が壊れるか」。zip に入らないものが壊れていても出力は壊れない。渡さないものを直さないと渡せない、という状態はゲートの趣旨と逆
    - **見分け方は既に決まっている**: ページ直下に `FRAME` か `SECTION` として置かれているか（4.7.4 の書き出し範囲の方針。実装は `src/export/exportScope.ts` の `isExportedFrame`）。Review が書き出しと**同じ関数**を使えば定義上ズレない。4.7.2 冒頭の「ページ内の全ノードを走査して」をこの範囲に書き換える
    - 除外物告知は既に「書き出し対象フレーム配下のみ」に絞ってあるので、両者の範囲がここで揃う
