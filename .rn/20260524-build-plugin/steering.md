...
Ph-9 の**設計書を最新化する段は D-5 の設計レビューを残すのみ**（2026-09-22）。D-0〜D-5 で未決を潰し、実装タスクを I-1〜I-9 として定義した。残る順は **D-5 の設計レビュー → W-1（実機検証）→ I-1 → I-2 →（I-3・I-4 は並行可）→ I-5 → I-6 → I-7 → I-8 → I-9**。D-4 と D-5 の4軸レビューはここまでの設計書をまとめて1回で当てる。
...
**結論（2026-09-22。ユーザー承認済み。`61b0470` で `docs/design.md` 4.7 に反映）**
...
- [ ] 常設ヘッダを作る: `Light | Dark` の segmented-control と error 件数。スクロールしても消えない位置に固定する
...
- [ ] テーマ整合チェック4種を足す。**発火判定は Export 設定の「ダークモード対応」に繋ぐ。Dark コレクションの有無で判定しない**（**D-4 (f)**）
...
- [ ] `figma.root.setPluginData` に保存し、次回の既定値として出す（4.7.4.3）
