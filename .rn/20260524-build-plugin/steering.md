...
**進捗（2026-09-22）**: 検証用の使い捨てプラグイン「W1 probe」（manifest / code.js / ui.html の3ファイル）を作り、実機操作をユーザーに依頼した段階で中断。ボタンは3つ — A: `radius/full`=9999 の FLOAT 変数を作り 200×80 の矩形の四隅に `setBoundVariable` して読み戻す（ピルに描かれるかは目視）、B: ファイル内の STRING Variable の `scopes` を列挙＋プラグインが作った直後の既定値も表示、後片付け: 作った矩形とコレクションを消す。**セッション用の一時ディレクトリに置いたので消えている可能性が高い。無ければ上の内容で作り直す**（リポジトリには置かない）。

### タスク定義の状態

Ph-9 の**設計書を最新化する段は D-5 まで完了**（2026-09-22）。D-0〜D-5 で未決を潰し、D-5 の設計レビュー（判定 fail・22件）を全件反映したうえで、実装タスクを I-1〜I-9 として定義した。残る順は **W-1（実機検証）→ I-1 → I-2 →（I-3・I-4 は並行可）→ I-5 → I-6 → I-7 → I-8 → I-9**。D-4 と D-5 の4軸レビューはここまでの設計書をまとめて1回で当てる。
