...
- [ ] **原則A/B を設計書に置く** — 4.3 の冒頭に原則A（Figma 公式を土台に、独自の約束は最小）、4.5 の冒頭に原則B（形が揃っている／1箇所にだけある／欠けていると分かる／推測させない）。以降の設計判断がここを根拠にできる状態にする
- [ ] Valid の分と、決着した未決3〜5・7〜13 を設計書に反映する（未決1・2 の決着後にまとめて。個別に書くと節をまたぐ整合が取れない）
...
- [ ] **未決1・2 を原則A/B から導いて結論を提示する**（ユーザー指示: 1件ずつ聞くのではなく結論と根拠を並べ、違うところだけ指摘してもらう）
  - 未決1（Notes タブの一覧を残すか Export に一本化するか）— 結論に応じて U-2 の残タスク（N-1〜N-5・N-10〜N-12）を定義し直す
  - 未決2（状態8種の受け口を Ph-9 に入れるか別フェーズに切るか）
...
* **Status**: paused
* **Date**: 2026-09-21
* **Last completed**: D-3 完了（設計書へのダーク対応・チェック体系・Export 設定の反映）。D-4 は設計レビュー（判定 fail、指摘12件）を回し、triage 済み。未決3〜5・7〜13 の11件を決着させ、各未決の節に結論と根拠を記録。あわせて「確定: 判断は原則から導く」（原則A: Figma 公式を土台に独自の約束は最小 / 原則B: 形が揃っている・1箇所にだけある・欠けていると分かる・推測させない）を新設した。**設計書への反映はまだ1行も書いていない**
* **Next**: D-4 の続き。① 未決1（Notes タブの一覧を Export に一本化するか）と未決2（状態8種の受け口を Ph-9 に入れるか）を**原則A/B から導いて結論と根拠を提示**する（1件ずつ聞かない。ユーザーは違うところだけ指摘する）。② 原則A/B を設計書 4.3 / 4.5 の冒頭に置く。③ 決着した未決と Valid 指摘をまとめて設計書に反映。④「D-3 で実装者が決めた点」8件の承認確認。⑤ 要検証3件の割り当て。⑥ Ph-9 の実装タスク定義
* **Notes**: ブランチ `worktree-figma-plugins` / PR https://github.com/lovaizu/telldes/pull/1。未追跡ファイルなし。要検証の `radius/full` の Variable 値（`9999` が通るか）は Figma 実機確認が未了で、Setup の実装タスク定義までに要る
