...
* **Status**: paused
* **Date**: 2026-09-25
* **Last completed**: W-1（実機検証）、I-1 の実装（`9042706`。実機確認は B-2 へ移した）、Ph-9 実装タスクの組み直し（旧 I-2〜I-10 → B-0〜B-9、ユーザー承認済み）
* **Next**: **B-0（作り直しの土台を設計書に書く）** から。Ph-8 の残りと T-1 は着手しない（吸収・廃止済み）
* **Notes**: 実機での確かめ方は、読み込み済みの Telldes に一時的なボタンを入れる（別プラグインにしない。コミットしない）
