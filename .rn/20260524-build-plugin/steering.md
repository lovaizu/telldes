...
**作業内容**:
- [ ] `radius/full` の Variable 値を `9999` にして、短辺がそれより小さい矩形にバインドする。Figma がクランプして丸く描くか、描画が壊れるかを確認する。壊れる場合は Setup が生成する `radius/full` の値を決め直す（設計書 4.3.4 の生成値に反映）
- [ ] STRING Variable を Figma の変数パネルで新規作成し、`scopes` の既定値を Plugin API で読む。`["ALL_SCOPES"]` かどうかを確認する
- [ ] 結果を本節に記録する。設計書の記述と食い違う場合はその場でユーザーにエスカレーションする
...
Ph-9 は「設計書を最新化 → 実装 → 試行」の**設計書を最新化する段**にある。未決は下の D-0〜D-5 で潰し、実装タスクはそれが終わってから定義する（内容が変わるため）。順は D-0 → D-1 → D-2 → D-3 → D-4 → **D-5（UI を設計し直す。2026-09-21 追加）** → 実装タスク定義。W-1（実機検証）は D-5 と並行してよい。
...
**作業内容**:
- [ ] 縦1画面＋見出し か、タブを残すか を決める（上の作業仮説の検証。Setup は1回きり、Review / Notes / Export は行き来する、という非対称の扱いを含む）
- [ ] 常に見えているものを決める（テーマの現在状態、error 件数）。`figma.notify()` に出すものとの線引きも決める — Figma の作法では結果の知らせは UI に領域を作らず notify に出す
- [ ] ウィンドウの寸法を決める（300px 幅で成立する形にするか、現行360を保つか）
- [ ] 未決1（note 一覧の置き場所）とテーマ切り替えトグルの配置を、決まった器の上で結論づける
- [ ] 決まった内容を `docs/design.md` 4.7 に反映する（4.7.1 の「4つのタブ」以下が書き換わる）
...
* **Status**: paused
* **Date**: 2026-09-21
* **Last completed**: D-4 の設計書反映まで（`789d577` ＋ 修正 `ffc73b1`）。原則A/B を 4.3 / 4.5 の冒頭に置き、決着済みの未決13件を `docs/design.md` に反映。未決1・2 を決着させ、状態8種を **Ph-10** として分離。要検証3件を割り当て、**W-1**（実機検証）を起票。さらに UI のベストプラクティスを調べた結果、**現行4タブが NN/g の条件に照らして誤用**と分かり、**D-5（UI を設計し直す）を起票**した
* **Next**: D-5 の作業仮説（縦1画面＋見出し、テーマ状態は常設、設定だけ disclosure）をユーザーが承認するか確認する。**あわせて回答待ちが2件ある** — ①「D-3 で実装者が決めた点」8件（うち #3 の `settings` 複製だけ差し替え推奨＝ルートに1つ）、② D-5 の作業仮説そのもの。承認が出たら D-5 → Ph-9 実装タスク定義（UI 系の (d)(g) は D-5 の後）→ D-4 と D-5 をまとめて4軸レビュー
* **Notes**: ブランチ `worktree-figma-plugins` / PR https://github.com/lovaizu/telldes/pull/1。未追跡ファイルなし。D-4 の4軸レビューは未実施 — D-5 で 4.7 が書き換わるため、そこまで済ませて1回で当てる
