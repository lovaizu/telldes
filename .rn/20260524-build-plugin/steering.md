...
* **何をどこに書くか**: 利用者の手順は `README.md`、なぜそうしたか（実装が変わっても変わらない判断）は `docs/design.md`、CC に渡すものの形と読み方は `src/templates/prompt.md`、細かい規則（変換・計算・判定）はコードとテスト。同じ事実を2か所に書かない。食い違ったら不具合として扱う。決めていない動作を勝手に足さない。根拠: 2026-09-25、設計書に出力の形・作りの細部・実機確認の一覧まで書き足し続け、1459行の読めない文書になった（ユーザー指摘「細かいロジックは実装やテストコード」）。設計書は開発者向けにし、登場人物と役割・構成・データの流れで全体像がつかめる形にする。デザイナー向けの作り方の理由は README の役目
...
* **Status**: paused
* **Date**: 2026-09-25
* **Last completed**: B-0 の設計書を開発者向けに組み直した（151行。最終 push 済み）
* **Next**: B-0 の「ユーザーが書き直した設計書を確認する」— ユーザーの `/rn:ty` / `/rn:gm` を待つ。承認なら B-0 を閉じて B-1 へ
* **Notes**: ブランチ `worktree-figma-plugins`。設計書への指摘は一件ずつ来ているので、直すときは同じ種類の問題を文書全体で探して一度に直す
