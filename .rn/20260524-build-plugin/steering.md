...
Ph-9 の**設計書を最新化する段は D-5 まで完了**（2026-09-22）。D-0〜D-5 で未決を潰し、D-5 の設計レビュー（判定 fail・22件）を全件反映したうえで、実装タスクを I-1〜I-9 として定義した。残る順は **W-1（完了）→ I-1（実機確認のみ残り、B-2 で行う）→ B-0〜B-9**（2026-09-25 に旧 I-2〜I-10 を組み直し。下の「Ph-9 実装の組み直し」）。D-4 と D-5 の4軸レビューはここまでの設計書をまとめて1回で当てる。
...
- [ ] セルフチェック → **B-2 の実機確認と一緒に行う**
- [ ] Figma 実機で試す → **B-2 に移す**（見本ファイルを作るときに Setup を実行するので、そこで確かめる）

**進捗（2026-09-25）**: 実装済み（`9042706`）。テスト14件は作り直し方針により B-0 で消す。残りは Figma での確認だけで、B-2 で行う。
...
* **今のコード**: 参考として読むだけで引き継がない。置き換えが済んだものから消し、B-9 で残りを消す。I-1 の Setup は作ったものを使う
...
順は B-0 → B-1 → B-2 →（B-3・B-4 は並行可）→ B-5 → B-6 → B-7 → B-8 → B-9。
...
**作業内容**:
- [ ] 現在のページと、ファイルの Variables / Style / Export 設定 / note を読む
- [ ] 読んだデータを JSON として保存する機能を付ける（B-2 で見本を取るため）
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-1.md`）
- [ ] Figma 実機で試す

**完了条件**:
- 実機で読み出した JSON に、B-0 で決めた項目がすべて入っていること

---

### B-2: 見本ファイル

**目的**: テストの入力になる実物の見本を作り、I-1 の Setup を実機で確かめる。

**前提**: B-1 完了

**作業内容**:
- [ ] 見本の中身を決めてユーザーに作成を依頼する（新規ファイルで Setup → ダーク対応の LP 1本、note、わざと入れた違反・除外物・付け忘れ）
- [ ] I-1 の実機確認: 変数44・Style 11 ができ、2回目で増えないこと、3コレクション、Text Style の太さが保たれること、影の色が `shadow` につながること、Dark がピッカーに出ないこと
- [ ] B-1 で読んだ JSON をリポジトリにテストの入力として保存する
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-2.md`）

**完了条件**:
- 見本の JSON がリポジトリにあり、入れた違反・除外物・付け忘れが一覧として記録されていること
- I-1 の完了条件がすべて実機で確かめられていること

---

### B-3: 出力を作る層

**目的**: 読んだデータから spec.json・tokens.json・README・settings.json・prompt.md・steering.md を作る（旧設計書 4.5・4.8）。

**前提**: B-2 完了

**作業内容**:
- [ ] ②の層として実装する（Figma に触らない）
- [ ] 見本を入力に、出力を正解と丸ごと比べるテストを書く。正解は旧設計書 4.5 の規則で作る
- [ ] CC が読む取り決め（旧設計書 4.5・4.6・4.8 のうち CC がすることの部分）を `prompt.md` に揃える。以後 CC への取り決めは `prompt.md` だけに書く
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-3.md`）

**完了条件**:
- 見本から作った出力が正解と一致し、正解が旧設計書 4.5・4.8 のフォーマットに合っていること

---

### B-4: チェック

**目的**: 違反・テーマ整合・付け忘れの知らせ・除外物を、旧設計書 4.7.2 の原則1〜3 どおりに出す（旧 I-4・I-7・U-3 を含む）。

**前提**: B-2 完了

**作業内容**:
- [ ] ②の層として実装する。走査範囲は書き出し対象（4.7.2）、行は原因単位
- [ ] 見本に入れた違反・除外物・付け忘れがすべて出て、それ以外は出ないことをテストする
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-4.md`）

**完了条件**:
- 見本に入れたものがすべて検出され、誤検出が無いこと
- ダークモード対応 OFF でテーマ整合が発火しないこと

---

### B-5: 画像の書き出し

**目的**: スクリーンショット（ライト・ダーク）・アセット・favicon / OG 画像（4.5.3）を書き出す。

**前提**: B-3 完了

**作業内容**:
- [ ] スクリーンショット・`screenshots-dark/`・アセット・`assets-dark/` を書き出す
- [ ] `site/` を書き出す。ICO は 32×32 の PNG を ICO コンテナに収める
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-5.md`）
- [ ] Figma 実機で試す

**完了条件**:
- 実機で書き出した画像が旧設計書 4.5 の命名・形式どおりで、`.ico` がブラウザで favicon として表示できること

---

### B-6: テーマ切り替え

**目的**: Light ⇄ Dark の付け替え（旧設計書 4.3.9）。

**前提**: B-2 完了

**作業内容**:
- [ ] どこをどう付け替えるかの計算を ②の層に置き、見本でテストする（ノードのバインド、Paint / Effect Style、グラデーション stop、影の色）
- [ ] 実際に書き換える部分を ①の側に置く。テーマ状態を `figma.root.setPluginData` に持つ
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-6.md`）
- [ ] Figma 実機で試す（往復で壊れないこと、Dark のまま開き直したときの復旧の促し）

**完了条件**:
- 実機で Dark にすると見た目がダークになり、Light に戻すとバインドが元と一致すること

---

### B-7: UI

**目的**: 旧設計書 4.7.1 の画面構成（トークン・画面・レイヤーの3ビュー、常設ヘッダ、Export 設定と確認ビュー、Setup）を作る（旧 I-2・I-5 の UI 部分・U-2）。

**前提**: B-3・B-4・B-6 完了

**作業内容**:
- [ ] Solid を `next` タグの最新（2.0 RC）に上げる（B-0 で承認済み。手順は U-8 節。実機確認はこのタスクの実機試験で兼ねる）
- [ ] `themeColors: true`・360×640、Figma の CSS 変数で追従
- [ ] 3ビュー・ヘッダ・件数の「未着／失敗／受信済み」・ページ切り替え時の作り直し
- [ ] Export 設定（保存・既定値・node id 参照）と確認ビュー（ゲートは「実行」に掛ける）
- [ ] 旧 UI（`src/App.tsx` 等）を消す
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-7.md`）
- [ ] Figma 実機で試す

**完了条件**:
- 旧設計書 4.7.1 の画面構成どおりに実機で動くこと
- error があっても確認ビューで設定を編集できること

---

### B-8: README の書き換え

**目的**: README「プラグインの使い方」と制作ルールを Ph-9 に合わせて書き直す（旧 I-8 の作業内容をすべて引き継ぐ）。

**前提**: B-7 完了

**作業内容**:
- [ ] 旧 I-8 の各項目（タブの記述廃止、Color Style の行、テーマ整合、zip ツリー、Export 設定、画面でないものは別ページへ、走査範囲、レスポンシブ、ダークモードの制作ルール、font-family の行、Setup の使い方）
- [ ] B-0 で旧設計書に足したもの: グラデーション・複数 fill の Color Style と Effect Style のトークン、tokens.json の出力条件、非表示のレイヤー（除外物）、線（strokes）、塗り・線の画像のファイル名（`{パス}--fill-{番号}` / `{パス}--stroke`）、新しい告知の種類（再現できない描画・決められない値）
- [ ] 旧設計書 4.3 / 4.5 / 4.7 と突き合わせる（README に要る「何を」を拾い切る）
...
**目的**: Setup → デザイン → Review → Export → CC を一度通し、旧コードの残りを消す（旧 I-9・T-1）。

**前提**: B-0〜B-8 完了

**作業内容**:
- [ ] 旧設計書 4.7.7「実機で確かめるもの」がすべて確かめられていることを確認する（B-1〜B-8 の実機試験で済んでいないものをここで確かめる）
- [ ] 見本とは別の LP で一度通し、zip を CC に渡してコーディングさせる
- [ ] ダークモードとレスポンシブが CC の出力に反映されることを確かめる
- [ ] 旧コードの残りを消す
- [ ] 出た不具合をステアリングに起票する
