...
* **正の置き場所**: 利用者の手順は `README.md`、なぜそうしたか（実装が変わっても変わらない判断）は `docs/design.md`、CC に渡すものの形と読み方は `src/templates/prompt.md`、細かい規則（変換・計算・判定）はコードとテスト。同じ事実を2か所に書かない。食い違ったら不具合として扱う。決めていない動作を勝手に足さない。根拠: 2026-09-25、設計書に出力の形・作りの細部・実機確認の一覧まで書き足し続け、1459行の読めない文書になった（ユーザー指摘「細かいロジックは実装やテストコード」）。設計書の形は rn の design-template（aiya の設計書が手本）に従う
* **Ph-9 の細かい規則の出どころ**: B-1〜B-9 の実装は、旧設計書（`git show d879d2c:docs/design.md`）の規則を読んで作る。コード・テスト・`prompt.md` に移したものは、以後そちらが正。旧設計書は書き換えない。設計書に書き足すのは、実装中に出た新しい判断のうち「なぜ」にあたるものだけ
...
- [ ] CC が読む取り決め（旧設計書 4.5・4.6・4.8 のうち CC がすることの部分）を `prompt.md` に揃える。以後 `prompt.md` が CC への取り決めの正になる
