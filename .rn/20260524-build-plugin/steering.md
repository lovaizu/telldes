...
7. **テーマ付け替えは何を対象にするのか**（レビュー #2 / #3 / #8）。3つが揃っていない — 付け替え範囲は「選択フレーム配下」（4.3.9）、状態は「ファイル単位」（4.3.9）、書き出しは「全フレーム」（4.7.4）。さらに **Effect Style とグラデーション Color Style のバインドはスタイルオブジェクト側に載るのでフレーム配下のノード走査では到達できない**（`setBoundVariableForPaint` は `SolidPaint` のみ、`ColorStop.boundVariables` は `readonly`）。にもかかわらず 4.7.2 は影の色の未バインドを error にしている。**「1フレーム＋トグル」方式が影とグラデーションまで届くのか**をここで決める
8. **Export 開始時が dark だったらどうするか**（レビュー #1）。4.7.4.4 の手順は開始時ライトを暗黙の前提にしているが、4.7.2 は dark 残りをゲートから明示的に外している。ゲートに入れる／Export の先頭で自動的に light に戻す／中止する、のいずれか
9. **発火表が README に送っている3項目をどうするか**（レビュー #4）。色の未バインド（OFF時）・font-size 生数値・padding/gap 生数値は、4.7.2 が定義する除外物（ツールが構造上捨てるもの）に当てはまらず、4.3.4 の「ツールは促さない」とも衝突する。表から落とす／除外物の定義を広げる／第3の出力を作る。「4の倍数」の基準も設計書に定義が無い
10. **favicon / OG 画像に指定したフレームを書き出し対象から外すか**（レビュー #5）。外さないと `favicon/spec.json` が出て CC が favicon をページとして実装する。ページ直下から外す運用をルールにする／指定されたフレームを書き出し対象から除く／別ページに置かせる
11. **ダーク用の `assets/` を出すか**（レビュー #6）。SVG は色が焼き込まれるので、出さないと黒背景に黒アイコンになる。`assets-dark/` を足す／アイコンは `currentColor` で出す運用をルールにする／ダークでは色を変えない前提にする
12. **`radius/full` の名前を守る仕組みを置くか**（レビュー #9）。フルパス一致が意味を持つのに 4.3.4 は「命名規約は自由」と言っており、改名で静かに効かなくなる。除外物告知に足す／名前ではなく別の印で宣言させる／自由のままにして告知しない
13. **Review の走査範囲を書き出し対象フレーム配下に絞るか**（レビュー #11）。今は「ページ内の全ノード」なので、書き出されない別エリアの裸 Component のベタ色で Export がブロックされる（原則1 の「出力が壊れるか」と逆）
...
3. `spec.json` の `settings` のキー名・フィールド名（`responsive` / `darkMode` / `site` / `rules`、`minWidth` / `frame` / `contentWidth` 等）と、全フレームの spec.json に同じ `settings` を載せること
...
* **ICO の生成手段**（D-3 で発覚）。Plugin API の書き出し形式は PNG / JPG / SVG / PDF のみで ICO が無い（`ExportSettings` を確認済み）。設計書 4.5.3 は「PNG を ICO コンテナに収める」とだけ書いており、手段は実装タスクで決める
* **テーマ切り替えトグルの UI 配置**（どのタブに置くか）。ステアリングにも設計書にも無い
...
- [ ] Valid の分を設計書に反映する（未決の決着待ち。決着した順に反映する）
- [ ] Ph-9 の実装タスクを定義する（ここで初めて定義できる）。**D-1 未決3・D-2 未決4 の決定により、実装タスクに必ず含めるもの**: (a) Setup が生成する変数への `scopes` 設定（GAP / CORNER_RADIUS / TEXT_FILL / FONT_FAMILY 等、Figma 公式の対応表通り。Dark コレクションの `scopes: []` とは別目的）、(b) G-3・U-1 の「Color Style 使用」検出を単色／複合（グラデーション・複数fill）で分岐する形に作り直す、(c) **README.md の Color Style 行の更新**（現在「使わない。使うと除外物として記録される」とあるが、未決4でグラデーションは正式な源泉になった。実装と同時に直さないと README と design.md が食い違う＝作業ルール上の不具合になる）、(d) **Export 設定 UI の実装**（設定4群・複数行の追加削除・`figma.root.setPluginData` への保存・note 一覧の最下段表示）と、それに伴う `spec.json` / `prompt.md` / zip 同梱 `steering.md` の更新、(e) **favicon / OG 画像の書き出し**（フレーム指定 → SVG / ICO / 180×180 / 1200×630）、(f) **テーマ整合チェックの発火判定を Export 設定のダークモード対応に繋ぐ**（Dark コレクションの有無で判定しない）

- [ ] **未決1〜13 をユーザーと決着させる（1件ずつ出す）**。順序は **未決7 → 8 → 11 → 9 → 13 → 10 → 12 → 3 → 5 → 4 → 1 → 2**（未決7 が他の前提。未決8・11 は7の結論に従属、未決3 は12番の Base 行の矛盾を併合）。決着した分から設計書に反映する
  - 未決1（Notes タブの一覧を残すか Export に一本化するか）の結論に応じて U-2 の残タスクを定義し直す
  - 未決3〜5（Setup のトークン数と Dark の扱い／Font family の除外物告知／Typography の段階数）は D-3 で発覚した設計書内の矛盾
  - 未決7〜13 は D-4 の設計レビューで出たもの。**未決7 は「1フレーム＋トグル」方式が影とグラデーションまで届くかという方式そのものの問題**
- [ ] **「D-3 で実装者が決めた点」8件をユーザーと確認し、承認か差し替えかを決める**。すでに設計書に入っているので、差し替えになった分は設計書を直す
- [ ] 要検証3件（`radius/full` の Variable 値 / ICO の生成手段 / テーマ切り替えトグルの UI 配置）の決着先を、実機検証タスクか実装タスクに割り当てる
