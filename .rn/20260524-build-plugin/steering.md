...
* チェック機能は旧設計書 4.7.2 節の全ルールを網羅し、偽陰性（見逃し）ゼロを目指す
* スクリーンショットとアセットの書き出しは、正しいノードが正しい形式・命名で出力されること
* zip 内の全ファイルが `prompt.md` の取り決め（元は旧設計書 4.5 節）に完全準拠すること
...
決めた仕様（なぜは `docs/design.md`、細かい規則は旧設計書 `d879d2c`）を動くプラグインとして実装する。目的は2つ。

1. Figma 上で制作ルール違反を検出し、デザイナーにフィードバックする
2. CC が正確にコーディングできるデザインスペック一式を zip で出力する

「動く」だけでは不十分。「決めた仕様に対して実装が1対1で対応しており、カバー漏れゼロである」ことを目指す。
...
* **何をどこに書くか**: 利用者の手順は `README.md`、なぜそうしたか（実装が変わっても変わらない判断）は `docs/design.md`、CC に渡すものの形と読み方は `src/templates/prompt.md`、細かい規則（変換・計算・判定）はコードとテスト。同じ事実を2か所に書かない。食い違ったら不具合として扱う。決めていない動作を勝手に足さない。根拠: 2026-09-25、設計書に出力の形・作りの細部・実機確認の一覧まで書き足し続け、1459行の読めない文書になった（ユーザー指摘「細かいロジックは実装やテストコード」）。設計書は開発者向けに、README や実装から読み取れない意図と決めたことだけを要点で書く（同日ユーザー指摘「図と説明、重複してない？」「設計書はポイントだけでいい」）。図と文で同じことを言わない。デザイナー向けの作り方の理由は README の役目
* **Ph-9 の細かい規則の出どころ**: B-1〜B-9 の実装は、旧設計書（`git show d879d2c:docs/design.md`）の規則を読んで作る。コード・テスト・`prompt.md` に移したものは、以後そちらに従う。旧設計書は書き換えない。設計書に書き足すのは、実装中に出た新しい判断のうち「なぜ」にあたるものだけ
* **進め方は「決める → 実装 → 試行」**: この順で回す。未決は実装の前に決め、判断の理由を設計書に書いてから実装に入る。実装が終わったら Figma 実機で試す
* **目的・意図はユーザー、実現方法は私が決める**: ユーザーに確認するのは、目的・意図が不明なとき、合意済みの方向（設計書の方針・作業の順）を変えるとき、環境を変えるときだけ。設計書の抜けや作り方の選択は、目的（出力の正確さ）と原則A/B から私が決めて設計書に書き、報告で伝える。未決を「未決」と書いたまま次へ進めない（決めてから進む）。根拠: 2026-09-20、Ph-8 と Ph-9 の優先順位を「未決」と書いたまま放置し、`/rn:up` が確認なしに先へ進めた。2026-09-25、目的から導ける設計書の抜け6件をユーザーに確認に出し、「目的や意図を確認して、実現方法はあなたが判断すべき」と指摘された
...
* **テストは Given / When / Then を明示する**: 各 `it` の中を `// given` / `// when` / `// then` の3ブロックに分け、空行で区切る。読んだ人が「何を用意し、何をして、何を確かめたか」を見出しだけで追える状態にする。`it` の名前は挙動を述べる文にする（既存どおり）。1行で済む単純なケースでも `// when` と `// then` は省かない。既存テストは揃えずに B-0 で消す（2026-09-25、作り直し方針）。リポジトリを問わない共通ルールとしては rn（ccpm）の Issue https://github.com/lovaizu/ccpm/issues/27 に起票済み — rn の execute 手順がテストの形を指定しておらず、verify 手順だけが GWT を見ている、という手順の穴。そちらが入ったら本項は不要になる
...
**2026-09-25: 着手しない。** B-9（通し試験）に吸収した。
...
**2026-09-25: 残りタスクには着手しない。** Ph-9 の組み直し（「Ph-9 実装の組み直し」節）に吸収または廃止した — U-2 → B-7 / U-3 → B-4 / U-5・U-6・U-7 → 廃止 / U-8 → B-0 で要否を決める。
...
Ph-9 の**設計書を最新化する段は D-5 まで完了**（2026-09-22）。D-0〜D-5 で未決を潰し、D-5 の設計レビュー（判定 fail・22件）を全件反映したうえで、実装タスクを I-1〜I-9 として定義した。残る順は **W-1（完了）→ B-0〜B-10**（2026-09-25 に旧 I-1〜I-10 を組み直し。I-1 の Setup も B-3 で作り直す。下の「Ph-9 実装の組み直し」）。D-4 と D-5 の4軸レビューはここまでの設計書をまとめて1回で当てる。
...
D-5 完了により UI の形が決まったので定義できるようになった。順は I-1 → I-2 →（I-3・I-4 は並行可）→ I-5 → I-6 → I-7 → I-8 → I-10 → I-9（I-10 は 2026-09-25 追加。I-9 の通し試験の前に済ませる）。W-1（実機検証）は I-1 より先に済ませる。
...
**作業内容**:
- [x] `src/setup/tokenFactory.ts` に、設計書 4.3.4 の一覧（Color Light 14 / Color Dark 14 / Spacing 8 / Radius 5 / Font family 3 ＝ 変数44、Text Style 9 / Effect Style 2 ＝ Style 11）を定数として持つ。値は仮置きで、隣り合う段階・役割どうしの違いが目で分かることだけを満たす（4.3.4）
- [x] `createTokens()` を実装する。`figma.variables.createVariableCollection()` で3コレクションを作り、`variable.setValueForMode()` で値を入れる
- [x] 各変数に `scopes` を設定する（**D-4 (a)**）。Figma 公式の対応表どおり — 余白は `GAP`、角丸は `CORNER_RADIUS`、色は用途別、書体は `["FONT_FAMILY"]`（4.3.4）。**Dark コレクションの全変数は `scopes: []`**（4.3.9。デザイナーのピッカーから消すためで、目的が別）
- [x] 全変数に `setVariableCodeSyntax('WEB', 'var(--<token-name>)')` を設定する
- [x] **冪等にする**: 同名のコレクション・変数・Style が既にあれば作り直さず、欠けているものだけ足す。2回押しても55個のままであること
- [x] Text Style 9個（`display` / `heading-lg` / `heading-md` / `heading-sm` / `lead` / `body` / `label` / `caption` / `code`）と Effect Style 2個を生成する。Text Style の書体は `font/*` に、Effect Style の影の色は `shadow` につなぐ
- [x] セルフチェック → **B-3 に置き換え**（2026-09-25。古いコードは B-2 で消し、Setup は B-3 で作り直す）
- [x] Figma 実機で試す → **B-3 に置き換え**

**進捗（2026-09-25）**: 実装済み（`9042706`）。テスト14件は作り直し方針により B-0 で消す。2026-09-25、ゼロから作り直す方針により B-3 に置き換え（このタスクは閉じる）。
...
### Ph-9 実装の組み直し（2026-09-25 決定）

**旧 I-2〜I-10 は B-0〜B-9 に置き換えた。** 経緯: 正は設計書なのに、コードを資産として扱い仕様変更のたびに継ぎ足してきた結果、目的のないテストが283件に積み上がり、直し切れないコードが残った（ユーザー指摘）。UI は丸ごと別物になり、書き出しの組み立ても Figma のプロパティを JSON に書き写す程度のロジック（全体で約3,900行）で、知見はコードではなく設計書にある。Ph-9 の設計は D-5 で固まったので、**設計書から一度に組み直す**。

* **今のコード**: 引き継がない。B-2 で最初に消し、読む層だけを残した空のプラグインから作る。I-1 の Setup も作り直す（B-3）
* **作り**: 3層 — ①Figma から読む層（薄く）②読んだデータから出力・チェック結果を作る層（Figma に触らない）③UI
* **テスト**: 設計書の約束（出力の形・チェックの規則・付け替えの計算）が守られていることだけを確かめる。②の層に、見本の Figma ファイル（実物）から取った入力を与え、結果を正解と丸ごと比べる。Figma でしか分からないことはテストにせず実機で確かめる。新しいテストは GWT で書く
* **Ph-8 の残りの行き先**: U-2 → B-7 / U-3 → B-4 / U-5・U-7 → 廃止 / U-8 → B-0 で要否を決める / U-6 → 廃止（Ph-9 完成時のレビューに一本化） / T-1 → B-9

順は B-0 → B-1 → B-2（片付けと土台）→ B-3〜B-7（機能をデザイナーが使う順に1つずつ完成させ、実機で試す。テストは書かない）→ B-8（テスト）→ B-9（README）→ B-10（通し試験）。

**2026-09-25 組み直し（ユーザー指摘）**: 古いコードを残したまま層ごとに横に作る順だったため、B-7 まで何も使えず、古い画面に足し続けて散らかった（「既存のコードとテストコードは捨てるのでは？」「solid.js も v2 の最新にするのでは？」「散らかってない？」）。最初に片付けて土台を作り、機能を1つずつ完成させる。テストは機能が一通り動いてから入れる（「3 は変更しまくるのでテストは無理。一通り動いたらテストを入れましょう」）。

---

### B-0: 作り直しの土台を設計書に書く

**目的**: 組み直しの前に、層の境目・テストの目的と仕組み・旧コードの扱いを設計書で決める。

**前提**: なし

**作業内容**:
- [x] 3層の境目を書く。特に ①読む層が ②に渡すデータの形（出力 4.5・4.8、チェック 4.7.2、テーマ付け替え 4.3.9 に要る情報がすべて入ること）
- [x] テストの目的と仕組みを書く（上の方針。見本ファイルから入力を取る手順、正解の更新の仕方、実機でしか確かめないものの一覧）
- [x] 旧 I-10 を決める: トークン名から CSS 変数名への規則（I-1 は仮に `/` → `-`）、デザイナーの命名との衝突（`fg/default` と `fg-default`）の扱い。設計書 4.5.1・`prompt.md`・Setup の `codeSyntax.WEB` を揃える
  - **2026-09-25 決定（ユーザー承認）**: CSS 変数名は telldes が決めて `tokens.json` に書き、CC に組み立てさせない。変数の `codeSyntax.WEB` に名前があればそれを出す（Setup が作る変数はすべてこれ）。無ければ Figma の既定と同じく `/` を `-` にして作る（Text Style は常にこちら）。名前の重なりのチェックは入れない（紛らわしい名前を両方作ったときだけ起き、まれなため）。レビューを受け、Style 由来の名前にはグループ名を付け、`.` は `-` にした（重なりの主な源を潰し、チェック無しの判断は維持）
- [x] U-8（Solid 2.0 RC への更新）の要否を決める。更新するならユーザー確認のうえ B-7 の前に
  - **2026-09-25 決定（ユーザー承認）**: 上げる。B-7 の最初に行う（画面を丸ごと作り直すので古いコードの書き換えが要らない）。手順は U-8 節のもの
- [x] 既存テスト（283件）を消す
- [x] 設計レビュー（実装前、設計書の文面を対象）
- [x] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-0.md`）
- [x] ユーザーが設計書の中身を確認する（2026-09-25 ユーザー指示。B-1 へ進む前）
  - **2026-09-25 差し戻し（ユーザー）**: 「人が読めるか」「設計書の目的は何か」「実装が変わるとすぐ影響を受けないか」「意図を書くだけでこの量が要るのか」。実態: 1459行のうち大半が CC 向けの出力の形と CSS への写し方（`prompt.md` と重複）、API 名・読み取りデータの項目・一時的な長方形の作り方などの作り、実機確認の一覧で、「なぜ」は埋もれている
- [x] 設計書の役割を決め直し、それに合わせて書き直す（2026-09-25 ユーザー指示: 細かいロジックは実装とテストへ、開発者向けに登場人物・構成・データの流れを持たせる）。1459行 → 151行（目的 / 前提と制約 / 考え方 / 全体像 / 仕組み / 捨てた案と代償）。推奨の語彙は README へ移した。細かい規則は旧設計書 `d879d2c` を B-1〜B-9 の出どころにする（作業ルール）
  - **2026-09-25 差し戻し（ユーザー）**: プラグインの機能名をボタンと同じ英語にそろえる（Setup / Review / Export / Light・Dark）。図と説明が重複しているので、README や実装から読み取れない意図と決めたことだけにする。151行 → 69行（目的 / 前提 / 決めたこと）。続けて「OOUI の話はどっか行ったの？」— 削りすぎていたので、画面の組み方の決定として要点を戻した
- [x] ユーザーが書き直した設計書を確認する
  - **2026-09-25 承認（ユーザー `/rn:ty`）**。README の Setup の使い方は、ボタンができる B-7 の後、B-8 で書く（使えない機能の使い方を先に書くと README が実物とずれる）

**完了条件**:
- 設計書が「なぜ」だけを持ち、人が上から読み通せること
- 読む層が渡すデータの形・テストの仕組み・CSS 変数名の規則が旧設計書 `d879d2c` で決まっていること（B-1〜B-9 の出どころ）
- 既存テストが残っていないこと
...
**目的**: Figma から出力・チェック・付け替えに要る情報をすべて読み、B-0 で決めた形（旧設計書 4.7.6「読み取りデータ」）のデータにまとめる。

**前提**: B-0 完了

**作業内容**:
- [x] 現在のページと、ファイルの Variables / Style / Export 設定 / note を読む（`src/readData.ts`・`src/read/`）
- [x] 読んだデータを JSON として保存する機能を付ける（古い画面のボタン。B-2 で土台の画面に移す）
- [x] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-1.md`）
- [x] Figma 実機で試す → **B-3 と B-6 に移す**（2026-09-25）。古い画面は B-2 で消すので、ここでは試さない。項目のそろいは B-3（Setup したファイル）、Export 設定・テーマ・`otherFrames` は B-6（設定を書き込めるようになってから）で確かめる。試し用の設定ボタン（`PROBE(B-1)`）は古い画面ごと B-2 で消える

**完了条件**:
- 読み取りデータの型と読む処理が、旧設計書 4.7.6 の3表の全行に対応していること（実機の JSON での確認は B-3・B-6）

---

### B-2: 片付けと土台

**目的**: 古いコードを消し、Solid を v2 の最新に上げ、読む層だけを持つ空のプラグインにする。以後の機能はこの上に1つずつ足す。

**前提**: B-1 完了

**作業内容**:
- [ ] Solid を `next` タグの最新に上げる（`bun update solid-js @solidjs/web vite-plugin-solid`、`bun.lock` 更新。B-0 で承認済み。beta → rc の破壊的変更は changelog で確認）
- [ ] 古いコードを消す: `src/checks/`・`src/export/`・`src/setup/`・`src/util/`・`src/App.tsx` と `src/code.ts`・`src/messages.ts` の古い処理。`src/readData.ts`・`src/read/`・`src/templates/`（CC に渡すひな形）は残す。消す前に、B-3 以降で要る知見（Setup の55個の一覧など）が旧設計書 `d879d2c` にあることを確かめる
- [ ] 空のプラグインにする: `figma.showUI` は 360×640・`themeColors: true`、画面は Figma の CSS 変数で色を追従させる。上部のヘッダの枠だけを置く（Setup・Review・`Light | Dark` の置き場所。中身は各機能のタスクで入れる）
- [ ] 読み取りデータを JSON で保存するボタンを、開発用として新しい画面に置く（テスト用の見本を取るため。B-10 で扱いを決める）
- [ ] `bun run build` と `bunx tsc --noEmit` が通ることを確かめる（JSX の型エラーが消えたかを記録する）
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-2.md`）
- [ ] Figma 実機で試す（開ける・ヘッダが出る・Figma のテーマを切り替えると画面の色が追従する・保存ボタンで JSON が保存される）

**完了条件**:
- `bun.lock` の3パッケージが `next` タグの同一系列にそろっていること
- `src/` に読む層・土台の画面・ひな形以外のコードが残っていないこと
- `bun run build` が通り、`tsc --noEmit` のエラーが0件であること
- 実機でプラグインが開き、Figma のライト・ダークに画面の色が追従すること

---

### B-3: Setup

**目的**: 作り方のガイドどおりの変数とスタイルの一式（旧設計書 4.3.4・4.3.9）を、ヘッダの Setup からファイルに作る。何度押しても増えない。

**前提**: B-2 完了

**作業内容**:
- [ ] Figma に触らない部分で、あるべき一式と読み取りデータにある今のものとの差（作るもの・設定し直すもの）を計算する（旧設計書 4.7.6「Setup の計画」）
- [ ] 読む層の側で、その差をそのまま Figma に書き込む
- [ ] ヘッダに Setup を置き、終わったら何ができたかと次にやること（Assets パネルで名前や値を変えられる）を知らせる
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-3.md`）
- [ ] Figma 実機で試す: 新しいファイルで Setup → もう一度 Setup → 保存ボタンで読み取りデータを保存し、B-1 の項目がそろっているか確かめる（レイヤーの種類ごとに、どの項目が出たかを一覧にして旧設計書 4.7.6 と突き合わせる）

**完了条件**:
- 実機で Setup を1回押したファイルに、変数44個と Style 11個がちょうどあること
- 2回目のあとも44個・11個のままで、値も変わっていないこと
- Light / Base の各変数の `scopes` が旧設計書 4.3.4 の表どおりで、Dark の全変数が `[]` であること
- 全変数の `codeSyntax.WEB` が `var(--<変数名>)` の形で入っていること
- Text Style の書体が `font/*` に、Effect Style の影の色が `shadow` につながっていること
- 実機で保存した読み取りデータに、旧設計書 4.7.6 の項目がすべて入っていること（Export 設定・テーマ・`otherFrames` は B-6）

---

### B-4: Review

**目的**: 違反・テーマ整合・付け忘れの知らせ・含まれなかったものを旧設計書 4.7.2 どおりに出し、トークン・画面・レイヤーの画面（4.7.1）に持ち主ごとに並べる。

**前提**: B-3 完了

**作業内容**:
- [ ] Figma に触らない部分でチェックを作る。見る範囲は書き出す範囲と同じ判定、行は原因単位
- [ ] トークンの一覧・画面の一覧（末尾に「渡されないもの」）・選んだレイヤーの画面を作る。行を押すとそのレイヤーを選ぶ
- [ ] プラグインを開いたとき・ページを切り替えたときに作り直す。件数の「まだ無い／失敗／受け取った」を区別し、まだ無いものを 0 と出さない
- [ ] ヘッダに Review と error 件数・付け忘れの件数を置く
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-4.md`）
- [ ] Figma 実機で試す（Setup したファイルに、わざと違反・付け忘れ・含まれなかったものを入れて、すべて出て、それ以外は出ないこと）

**完了条件**:
- 実機で、入れた違反・付け忘れ・含まれなかったものがすべて持ち主の行に出て、誤って出るものが無いこと
- ダーク対応 OFF のファイルでテーマ整合が出ないこと

---

### B-5: note

**目的**: 選んだレイヤーの画面で note を読み書きし、画面の一覧に note の件数を出す（旧設計書 4.3.7・4.7.3）。

**前提**: B-4 完了

**作業内容**:
- [ ] レイヤーの画面に note の入力欄を置き、保存する。note のあるレイヤーに Figma の「Edit note」ボタンを出す
- [ ] 画面の一覧に note の件数を出し、展開したレイヤーから選べるようにする
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-5.md`）
- [ ] Figma 実機で試す（書く・消す・「Edit note」から開く・書き出し範囲の外のレイヤーに書くと「渡されないもの」に出る）

**完了条件**:
- 実機で note を書き、開き直しても残っていること
- 書き出し範囲の外のレイヤーの note が「渡されないもの」に出ること

---

### B-6: Export

**目的**: Export 設定と確認の画面を作り、zip（spec.json・tokens.json・settings.json・README.md・prompt.md・steering.md・画像）を旧設計書 4.5・4.8 どおりに書き出す。

**前提**: B-5 完了

**作業内容**:
- [ ] Export 設定（4群）を `figma.root` に保存する。書き込みは `src/readData.ts` の `ExportSettings` の取り決め（無い欄の意味）に従う。フレームを選べるのは各ページ直下のフレームだけ（`otherFrames` はそのノード自身の `visible` しか持たないため）
- [ ] 確認の画面（設定 → 共通ルール → 付け忘れの件数 → 実行 → ダウンロード）。実行の前に Review を走らせ、error があれば止める。error があっても設定は編集できる
- [ ] Figma に触らない部分で zip の各ファイルの中身と、撮る画像の一覧を作る。ファイルの外（ライブラリ）の変数・スタイルを指す id の扱いを決める（削除されたものと区別できない）。`images` はページ全体の分が来るので書き出し範囲で絞る
- [ ] 読む層の側で画像を撮る（スクリーンショット・アセット・`site/`。ICO は 32×32 の PNG を ICO の入れ物に収める）
- [ ] CC が読む取り決め（旧設計書 4.5・4.6・4.8 のうち CC がすること）を `prompt.md` にそろえる。以後 CC への取り決めは `prompt.md` だけに書く
- [ ] 保存された設定が壊れているとき（読み取り全体が止まる）に、画面から設定を消して直せるようにする
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-6.md`）
- [ ] Figma 実機で試す（設定を入れて書き出し、zip の中身を旧設計書 4.5 と突き合わせる。別ページの favicon・消したフレーム・レスポンシブの行が指すフレームで、読み取りデータの `settings`・`theme`・`otherFrames` を確かめる）

**完了条件**:
- 実機で書き出した zip のファイル・画像が旧設計書 4.5・4.8 の形と名前どおりであること
- `.ico` がブラウザで favicon として表示されること
- 読み取りデータの `settings`・`theme`・`otherFrames` に実際の値が入り、別ページのフレーム・見つからないフレームが見分けられること

---

### B-7: Light / Dark

**目的**: ヘッダの `Light | Dark` でページ全体の変数のつながりを付け替え、ダーク対応の Export で `screenshots-dark/`・`assets-dark/` を撮る（旧設計書 4.3.9・4.7.4.4）。

**前提**: B-6 完了

**作業内容**:
- [ ] Figma に触らない部分で付け替えの計画（ノードのつながり、Paint / Effect Style、グラデーションの stop、影の色）を作り、読む層の側でそのまま書き込む
- [ ] テーマの状態を `figma.root` に持ち、ヘッダに常に出す。Dark のまま開いたら戻すよう促す
- [ ] Export を「Light で撮る → Dark に付け替えて撮る → Light に戻す」にする。途中で失敗しても必ず Light に戻す
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-7.md`）
- [ ] Figma 実機で試す（往復で壊れないこと、Dark のまま開き直したときの促し、ダーク対応の Export）

**完了条件**:
- 実機で Dark にすると見た目がダークになり、Light に戻すとつながりが元と一致すること
- ダーク対応の Export で `screenshots-dark/` が出ること

---

### B-8: テストの仕組み

**目的**: 一通り動いた機能に、旧設計書 4.7.7 のテスト（見本から取った読み取りデータを Figma に触らない部分に与え、正解と丸ごと比べる）を入れる。

**前提**: B-7 完了

**作業内容**:
- [ ] 見本の中身を決めてユーザーに作成を依頼する（新しいファイルで Setup → ダーク対応の LP 1本、note、わざと入れた違反・含まれなかったもの・付け忘れ）
- [ ] 保存ボタンで取った読み取りデータをリポジトリに置く
- [ ] Setup の差・Review・zip の中身・付け替えの計画を、正解と丸ごと比べるテストを GWT で書く。正解は今の出力を写さず、`prompt.md` と見本に入れたものの一覧から作る
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-8.md`）

**完了条件**:
- 見本の読み取りデータがリポジトリにあり、入れたものが一覧として記録されていること
- Setup・Review・Export・Light / Dark の計算がそれぞれ正解と丸ごと比べられ、通ること

---

### B-9: README

**目的**: README の作り方と使い方を、できあがったプラグインに合わせて書き直す（旧 I-8 の作業内容を引き継ぐ）。

**前提**: B-8 完了

**作業内容**:
- [ ] 旧 I-8 の各項目（タブの記述をやめる、Color Style の行、テーマ整合、zip の中身、Export 設定、画面でないものは別ページへ、見る範囲、レスポンシブ、ダークモードの作り方、書体の行、Setup の使い方）
- [ ] B-0 で旧設計書に足したもの（グラデーション・複数 fill の Color Style と Effect Style のトークン、tokens.json の出力条件、非表示のレイヤー、線、塗り・線の画像のファイル名、再現できない描画・決められない値）
- [ ] 旧設計書 4.3・4.5・4.7 と突き合わせる
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-9.md`）

**完了条件**:
- README だけを読んで Setup から Export まで操作でき、設計書・`prompt.md` と食い違う記述が無いこと

---

### B-10: 通し試験と片付け

**目的**: Setup → デザイン → Review → Export → CC を一度通す（旧 I-9・T-1）。

**前提**: B-9 完了

**作業内容**:
- [ ] 旧設計書 4.7.7「実機で確かめるもの」がすべて確かめられていることを確認する
- [ ] 見本とは別の LP で一度通し、zip を CC に渡してコーディングさせる。ダークモードとレスポンシブが反映されることを確かめる
- [ ] 開発用の保存ボタンを残すか（開発ビルドだけに出すか）を決めて片付ける
- [ ] 出た不具合をステアリングに起票する
- [ ] Ph-9 完成時の QA / Craft / Verification レビュー（作業ルール「レビューの配置」）
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-10.md`）

**完了条件**:
- 一度通り、CC の出力にダークモードとレスポンシブが反映されていること
- 出た不具合がすべて起票されていること
