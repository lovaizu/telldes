...
### I-10: トークン名から CSS 変数名への規則を決めて CC に伝える

**目的**: Setup が Dev Mode に出す名前（`codeSyntax.WEB`）と、CC が `tokens.json` から作る CSS 変数名を一致させる。I-1 で発覚（2026-09-25）— 規則がどこにも書かれておらず、CC は `fg/default` を何という変数にするか推測することになる（原則B「推測させない」違反）。I-1 は仮に「`/` を `-` に置き換える」（`fg/default` → `var(--fg-default)`）で実装した。

**前提**: I-1 完了

**作業内容**:
- [ ] 規則を設計書（4.5.1 の tokens.json の節）に書く。I-1 の仮の規則でよいか、デザイナーの命名に `-` が含まれる場合の衝突（`fg/default` と `fg-default`）をどう扱うかを含めて決める
- [ ] `prompt.md` に規則を1行で書く
- [ ] I-1 の `webCodeSyntax()` が規則と一致していることを確かめる（食い違えば直す）
- [ ] セルフチェック（完了条件ごとに OK/NG。チェック結果: `.rn/20260524-build-plugin/checks/I-10.md`）

**完了条件**:
- 設計書・`prompt.md`・Setup の `codeSyntax.WEB` の3か所が同じ規則になっていること
- 名前の衝突時の扱いが設計書に書かれていること
