...
**作業内容**:
- [ ] 3層の境目を書く。特に ①読む層が ②に渡すデータの形（出力 4.5・4.8、チェック 4.7.2、テーマ付け替え 4.3.9 に要る情報がすべて入ること）
- [ ] テストの目的と仕組みを書く（上の方針。見本ファイルから入力を取る手順、正解の更新の仕方、実機でしか確かめないものの一覧）
- [ ] 旧 I-10 を決める: トークン名から CSS 変数名への規則（I-1 は仮に `/` → `-`）、デザイナーの命名との衝突（`fg/default` と `fg-default`）の扱い。設計書 4.5.1・`prompt.md`・Setup の `codeSyntax.WEB` を揃える
  - **2026-09-25 決定（ユーザー承認）**: CSS 変数名は telldes が決めて `tokens.json` に書き、CC に組み立てさせない。変数の `codeSyntax.WEB` に名前があればそれを出す（Setup が作る変数はすべてこれ）。無ければ Figma の既定と同じく `/` を `-` にして作る（Text Style は常にこちら）。名前の重なりのチェックは入れない（紛らわしい名前を両方作ったときだけ起き、まれなため）
...
- [ ] セルフチェック（チェック結果: `.rn/20260524-build-plugin/checks/B-0.md`）
...
* **Last completed**: #N 説明
* **Next**: #N 説明
* **Notes**: 次の一手・未解決の障害・保留中の判断への短い手がかり（経緯は `git log` にある）
