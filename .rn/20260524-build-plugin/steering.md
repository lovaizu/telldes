...
* **何をどこに書くか**: 利用者の手順は `README.md`、なぜそうしたか（実装が変わっても変わらない判断）は `docs/design.md`、CC に渡すものの形と読み方は `src/templates/prompt.md`、細かい規則（変換・計算・判定）はコードとテスト。同じ事実を2か所に書かない。食い違ったら不具合として扱う。決めていない動作を勝手に足さない。根拠: 2026-09-25、設計書に出力の形・作りの細部・実機確認の一覧まで書き足し続け、1459行の読めない文書になった（ユーザー指摘「細かいロジックは実装やテストコード」）。設計書の形は rn の design-template（aiya の設計書が手本）に従う
...
- [x] 設計書の役割を決め直し、それに合わせて書き直す（2026-09-25 ユーザー指示: aiya の設計書を手本に、細かいロジックは実装とテストへ）。rn の design-template の5節で「なぜ」だけに絞った（1459行 → 約220行）。推奨の語彙は README へ移した。細かい規則は旧設計書 `d879d2c` を B-1〜B-9 の出どころにする（作業ルール）
- [ ] ユーザーが書き直した設計書を確認する
...
* **Status**: paused
* **Date**: 2026-09-25
* **Last completed**: B-0 の設計書の書き直し（「なぜ」だけ、rn design-template の5節。細かい規則は旧設計書 `d879d2c` が出どころ）
* **Next**: B-0 の「ユーザーが書き直した設計書を確認する」— `/rn:ty` なら B-0 を閉じて B-1 へ
* **Notes**: 設計書は上から通して読む前提で書いた
