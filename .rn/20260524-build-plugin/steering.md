...
- [ ] **コンテナ中央寄せパターンを HTML 導出ルール（4.3.6）に追記する**（2026-09-20 の実データ検証で発見）。左右対称の大きな padding（実例: 1440px フレームに `padding: 0 340px`）は「間隔」ではなく「中身を 760px に制限して中央寄せする」という意図であり、CC がそのまま `padding: 0 340px` と出力するとレスポンシブで破綻する。`max-width` + `margin: 0 auto` に変換するルールとして書く。変換の判定条件（左右 padding が等しく、かつ一定値以上か）も決める
- [ ] **角丸の `full` 変換ルールを設計書に記載する**（同上）。Figma の corner radius は常に絶対 px でパーセント指定が無いため、円・ピル形状は「要素の短辺の半分」の px 値として現れる（実例: 100×100px の要素に radius 50、カルーセルの角に 80）。この px 値をそのまま出力せず `border-radius: 9999px`（正方形なら `50%`）に変換するルールとして書く。**要検証**: 半径が短辺の半分を超える値を設定したとき Figma 側がどう描画・保持するか（クランプするか）を実機で確認してから確定する

**完了条件**:
- 上記の各点が `docs/design.md` に記載され、ステアリングの「確定:」2節が設計書を指すだけの状態になっていること
- チェック体系の各チェックについて、ダーク対応ファイル／ライトのみファイルでの発火有無が表として設計書にあること
- コンテナ中央寄せと角丸 `full` の2つの導出ルールが、変換の判定条件つきで 4.3.6 に記載されていること
