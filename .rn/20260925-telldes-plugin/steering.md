...
作り方は、土台（最新のビルド設定・Solid v2）を作り、作り込む前に画面だけを試して固めてから、デザイナーが使う順（Setup → Review → note → Export → Light / Dark）に機能を1つずつ完成させ、そのつど実機で試す。一通り動いたらテストを入れ、ガイドを実物と突き合わせ、通し試験をする。README・設計書・ガイドは実装が従う目指す姿で、変えるときは文書を先に変える。
...
- README・設計書・ガイドが目指す姿を書き（`.rn/doc-review.md`）、実装がそれに従っていて、3つが食い違わない
...
- README と設計書のすべての文が、`.rn/doc-review.md` の判定を通る（実装が従うべき目指す姿で、実装が自由に選んでよいことが無い）
- 経緯を知らない読み手が、文書ごとの観点で目的を果たせる（README で使うかを決めてガイドへ進める、設計書で README の姿の実現方法が分かり、ある変更が設計に合うかを言える、ガイドで zip を作れる）
- 3つの文書に食い違いが無い。実装済みの画面と違うところは、文書を正として後のタスクに直しを入れる
...
- [ ] README・設計書が、できあがったものと食い違わないかを確かめる（文書が正。文書を変えるべきと思えば、ユーザーに聞いてから文書を先に変える）
